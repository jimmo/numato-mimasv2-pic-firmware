...
parser.add_argument('--fast', help='Use faster writes', default=True, type=bool)
...
def small_write(addr, data):
  if len(data) < 16:
    data += b'\x00' * (16-len(data))
  send_command(SPI_COMMAND_WRITE, struct.pack('<I', addr) + data, 0)

def checksum_page(addr):
  status, checksum = send_command(SPI_COMMAND_CHECKSUM_PAGE, struct.pack('<I', addr), 4)
